...
try {
    ...
    Assert-True -Condition ($studyPromptResult.Output -match '(?s)До подробного разбора первого модуля.*общую карту всей заявленной области' -and $studyPromptResult.Output -match '(?s)Если нужны только материалы.*без требования отвечать сейчас.*В режиме совместного обучения затем начни первый подробный блок') -Message 'rendered study prompt must put the whole-system map before detailed lessons and keep materials-only work independent of learner replies'
